...
class Worker:
  def __init__(
    ...
    self.net = create_networks(net.num_outputs,name)
    ...
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          #self.init_feature = self.last_lstm_feature
  ...
  def sample_action(self, state, sess,last_feature):
    # Make input N x D (N = 1)
    feed_dict = { self.net.states: [state],self.net.h:last_feature[0],self.net.c:last_feature[1] }
    actions ,expected_value,hct= sess.run([self.net.logits,self.net.vhat,self.net.hct], feed_dict)
    v = expected_value[0][0]
    a = np.random.choice(self.net.num_outputs, p=actions[0])
  ...
  def run_n_steps(self, n, sess):
    ...
    for _ in range(n):
      ...
      # Take a step
      action,value,self.last_lstm_feature = self.sample_action(self.state, sess,self.last_lstm_feature)
  ...
  def update(self, steps, sess):
    ...
    if not steps[-1].done:
      _,reward,_=self.sample_action(steps[-1].next_state,sess,self.last_lstm_feature)
