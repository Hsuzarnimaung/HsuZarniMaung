...
def get_copy_params_op(src_vars, dst_vars):
  with tf.variable_scope(dst_vars):
    sv = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=src_vars)
    dv = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=dst_vars)
...
def make_train_op(local_net, global_net):
  ...
  p_local_grads, _ = tf.clip_by_global_norm(p_local_grads, 5.0)
...
class Worker:
  def __init__(
    ...
    with tf.variable_scope(name):
      ...
      self.init_feature = [np.zeros((1, 256)), np.zeros((1, 256))]
      self.h = np.zeros((1, 256))
      self.c = np.zeros((1, 256))

    # We will use this op to copy the global network weights
    # back to the local policy and value networks
    self.copy_params_op = get_copy_params_op("global",self.name
      )
    ...
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          self.init_feature=[self.h,self.c]
          if(steps[-1].done):
            #print("HI")
  ...
  def sample_action(self, state, sess,ht,ct):
    # Make input N x D (N = 1)
    feed_dict = { self.net.states: [state] ,self.net.h:ht,self.net.c:ct}
  ...
  def run_n_steps(self, n, sess):
    ...
    for _ in range(n):
      ...
      # Take a step
      action,value = self.sample_action(self.state, sess,self.h,self.c)
  ...
  def update(self, steps, sess):
    ...
    if not steps[-1].done:
      _,reward,_,_=self.sample_action(steps[-1].next_state,sess,self.h,self.c)
    ...
    feed_dict = {
      ...
      self.net.h:self.init_feature[0],
      self.net.c:self.init_feature[1]
