...
NUM_WORKERS =8 # This one run with 12 processors
...
with tf.device("/cpu:0"):
    ...
    with tf.variable_scope("global") as vs:
        net = create_networks(NUM_ACTIONS,"global")
...
with tf.Session() as sess:
    if (False):
        print('Loading Model...')
        ckpt = tf.train.get_checkpoint_state("./Model/DemonAttack_SpaceInvader_")
    ...
    saver.save(sess, "./Model/DemonAttack_SpaceInvader_" + "/model" + str("_global_step") + ".cptk")
    end = datetime.now()
    print(f"Training time:{end - start}")
    df = pd.DataFrame.from_dict({"Rewards": returns_list,"Global_Step":step_list})
    df.to_excel("./Excel/Hybrid_DemonAttack_lstm.xlsx", index=False)
    df = pd.DataFrame.from_dict({"Rewards": returns_list2,"Global_Step":step_list2})
    df.to_excel("./Excel/Hybrid_SpaceInvaders_lstm.xlsx", index=False)
