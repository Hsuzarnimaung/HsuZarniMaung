...
with tf.Session() as sess:
    if (False):
        print('Loading Model...')
        ckpt = tf.train.get_checkpoint_state("Hybrid_A3C/SpaceInvaders_DemonAttack")
    ...
    saver.save(sess,"Hybrid_A3C/Breakout_Pong/model-"+str(MAX_GLOBAL_STEPS)+".cptk`")
    df=pd.DataFrame.from_dict({"Rewards":returns_list,"Global Step":steps_list1})
    df.to_excel("Hybrid_Breakout_lstm.xlsx",index=False)
    df=pd.DataFrame.from_dict({"Rewards":returns_list2,"Global Step":steps_list2})
    df.to_excel("Hybrid_Pong_lstm.xlsx",index=False)
    end = datetime.now()
