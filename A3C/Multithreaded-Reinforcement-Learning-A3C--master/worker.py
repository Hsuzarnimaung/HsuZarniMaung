...
class Worker:
  def __init__(
    ...
    self.steps=0
    self.policy_net_ht = np.zeros((1,256))
    self.policy_net_ct = np.zeros((1,256))
    self.value_net_ht = np.zeros((1, 256))
    self.value_net_ct = np.zeros((1, 256))
  ...
  def run(self, sess, coord, t_max,saver):
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          steps, global_step = self.run_n_steps(t_max, sess,saver)
          ...
          if steps[-1].done:
            self.value_net.ht = tf.zeros((1, 256))
            self.value_net.ct = tf.zeros((1, 256))
            self.policy_net.ht = tf.zeros((1, 256))
            self.policy_net.ct = tf.zeros((1, 256))
  ...
  def run_n_steps(self, n, sess,saver):
    steps = []
    for _ in range(n):
      # Take a step
      action = self.sample_action(self.state, sess)
      #print(action)
      next_frame, reward, done, _ = self.env.step(action[0])
     ...
     # if len(self.returns_list) > 5000:
      ...
      if done:
        ...
        if len(self.returns_list) > 0 and len(self.returns_list) % 100 == 0:
          print("*** Total average reward (last 100):", np.mean(self.returns_list[-100:]), "Collected so far:", len(self.returns_list),"Global_step:",self.global_counter)
        self.total_reward = 0.
        saver.save(sess, self.model_path + '/model-' + str(self.global_counter) + '.cptk')

        self.steps_list.append(self.steps)
      else:
        self.total_reward += reward
      #print(self.policy_net_ct)
