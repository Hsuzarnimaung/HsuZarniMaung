      ...
      try:
        while not coord.should_stop():
          ...
          if(steps[-1].done):
            ...
            if len(self.returns_list) > 0 and len(self.returns_list) % 2000 == 0:
              saver.save(sess, "SpaceInvaders_DemonAttack"+str(len(self.returns_list))+"/model-" + str(len(self.returns_list)) + ".cptk`")
  ...
  def run_n_steps(self, n, sess):
    ...
    #   after 100 episodes start to render worker
      ...
      if done:
        ...
        print("Total reward:", self.total_reward, "Worker:", self.name,"Episode Steps:",self.length)
        self.length=0
        ...
        if len(self.returns_list) > 0 and len(self.returns_list) % 100 == 0:
          print("*** Total average reward (last 100):", np.mean(self.returns_list[-100:]), "Collected so far:", len(self.returns_list), "Total Count:",self.global_counter)
        self.total_reward = 0.
